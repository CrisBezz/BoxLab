...
source = r'''// BoxLab v0.36.18.274 — explicit quality guard for SubD-friendly unequal open-chain Bridge.
...
'''
Path('src/bridge-open-chain-all-quad.js').write_text(source)

boot=Path('src/loose-bootstrap.js')
b=boot.read_text().replace("bridge-open-chain-all-quad.js?v=0.36.18.273","bridge-open-chain-all-quad.js?v=0.36.18.274")
boot.write_text(b)

test=Path('tests/bridge-open-chain-all-quad.test.mjs')
t=test.read_text()
t=t.replace("installOpenChainAllQuadBridge,canTryOpenAllQuad,densifyOpenChainToCount", "installOpenChainAllQuadBridge,canTryOpenAllQuad,densifyOpenChainToCount,validateOpenAllQuadCandidate")
extra=r'''
...
test('274 fallback records reason for fold-prone L-shaped all-quad attempt',()=>{
  ...
  assert.ok(['folded-quad','degenerate-quad','connector-distortion'].includes(globalThis.__boxlabOpenChainAllQuadBridge?.lastReject));
});
'''
if "274 guard reports folded quad explicitly" not in t:t+=extra
test.write_text(t)
