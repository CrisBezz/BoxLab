// BoxLab v0.36.18.239 — segmentation-independent final Extrude/Through integrity.
// Validates the actual active object's final mesh after the direct controller and
// Object Manager refreshes finish. Mesh object identity is intentionally irrelevant.
import {gateClosedEdit,topologySummary} from './topology-seam-conformance.js?v=0.36.18.239';
...
function targetMesh(current){
  const m=manager();
  if(m?.activeId===current.objectId&&liveMesh())return liveMesh();
  m?.saveActive?.();
  return (m?.objects||[]).find(object=>object.id===current.objectId)?.mesh||null;
}
function begin(event){
  if(event.target!==canvas||!event.isPrimary)return;
  const tool=armedTool(),mesh=liveMesh(),m=manager();if(!tool||!mesh?.clone||!m)return;
  ...
  session={pointerId:event.pointerId,tool,objectId:m.activeId,before,faces:selectedFaces(),undo:cloneStack(h?.undoStack),redo:cloneStack(h?.redoStack)};
}
function validate(current){
  const mesh=targetMesh(current);
  if(!mesh||!changed(current.before,mesh)){
  ...
  if(gated.ok&&gated.after?.closed){
    restore(mesh,gated.mesh);manager()?.saveActive?.();render();globalThis.__boxlabTopologyGate?.sync?.();
    if(status)status.textContent=`Extrude Through • global seam conformance • ${gated.splits||0} split${gated.splits===1?'':'s'} • CLOSED`;
    return;
  }
  restore(mesh,current.before);restoreHistory(current);bridge()?.set?.('face',current.faces);manager()?.saveActive?.();render();globalThis.__boxlabTopologyGate?.sync?.();
  if(status)status.textContent='Extrude Through • rollback • open topology refused';
}
function finish(event){
  if(!session||session.pointerId!==event.pointerId)return;
  const current=session;session=null;
  if(event.type!=='pointerup')return;
  // Run after same-event microtasks and Object Manager save/refresh work.
...
globalThis.__boxlabDirectCommitIntegrity239={version:VERSION,topologySummary,get last(){return last;}};
