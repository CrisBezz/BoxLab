...
function decorateHierarchy(){
  ...
  try{
    for(const [groupId,rows] of grouped){
      ...
      const name=document.createElement('button');name.type='button';name.className='boxlab-group-name';name.textContent=groupLabel(groupId);name.title='Select whole group';name.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();selectGroup(groupId,{additive:multiEnabled&&selectedIds.size>0});});
