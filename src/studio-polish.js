...
function makeContactTexture(){
  ...
  gradient.addColorStop(.28,'rgba(0,0,0,.28)');
...
function ensureContact(rig){
  ...
  contact=rig.getObjectByName('BoxLab Studio Contact Shadow');
  ...
  contact=new THREE.Mesh(new THREE.PlaneGeometry(1,1),material);
  contact.name='BoxLab Studio Contact Shadow';
  contact.rotation.x=-Math.PI/2;
  contact.renderOrder=2;
  contact.userData.boxlabStudioPolish=true;
  rig.add(contact);
  return contact;
}
...
function apply(){
  ...
  scene.traverse(object=>{
    ...
    bounds.expandByObject(object);found=true;
  });
  if(!found||bounds.isEmpty())return true;
  const center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3());
  const shadow=ensureContact(rig);
  const span=Math.max(size.x,size.z,1);
  shadow.position.set(center.x,bounds.min.y-.012,center.z);
  shadow.scale.set(span*1.55,span*1.55,1);
  shadow.visible=true;
  return true;
}
...
function schedule(){
  ...
  requestAnimationFrame(()=>{
    if(!apply())setTimeout(()=>{queued=false;apply();},120);
  });
}

document.addEventListener('boxlab-render-mode-change',event=>{
  if(event.detail?.mode==='studio')schedule();
  else if(contact)contact.visible=false;
});
window.addEventListener('boxlab-bridge-state',schedule);
document.addEventListener('pointerup',()=>{if(studioActive())schedule();},true);
[0,120,500,1200].forEach(delay=>setTimeout(schedule,delay));
