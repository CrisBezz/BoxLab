...
function decorateHierarchy(){
  ...
  try{
    for(const [groupId,rows] of grouped){
      ...
      const ungroup=document.createElement('button');ungroup.type='button';ungroup.textContent='U';ungroup.title='Ungroup';ungroup.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();selectGroup(groupId);document.querySelector('#objectGroupTools [data-group-action="ungroup"]')?.click();});
      header.append(collapse,name,visible,lock,ungroup);block.append(header,children);first.before(block);for(const row of rows)children.append(row);
    }
