...
function decorateHierarchy(){
  ...
  try{
    for(const [groupId,rows] of grouped){
      ...
      const collapse=document.createElement('button');collapse.type='button';collapse.textContent=collapsedGroups.has(groupId)?'▸':'▾';collapse.title=collapsedGroups.has(groupId)?'Expand group':'Collapse group';collapse.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();collapsedGroups.has(groupId)?collapsedGroups.delete(groupId):collapsedGroups.add(groupId);updateUI();});
      ...
      const visible=document.createElement('button');visible.type='button';const members=groupMembers(groupId),allHidden=members.length&&members.every(o=>!o.visible);visible.textContent=allHidden?'–':'V';visible.title=allHidden?'Show group':'Hide group';visible.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const hide=groupMembers(groupId).some(o=>o.visible);for(const o of groupMembers(groupId))o.visible=!hide;forceRender();updateUI();});
      const lock=document.createElement('button');lock.type='button';const editableMembers=members.filter(o=>o.kind!=='reference'),allLocked=editableMembers.length&&editableMembers.every(o=>o.locked);lock.textContent=editableMembers.length?(allLocked?'L':'○'):'R';lock.title=editableMembers.length?(allLocked?'Unlock editable group members':'Lock editable group members'):'Reference-only group • read-only';lock.disabled=!editableMembers.length;lock.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const editable=groupMembers(groupId).filter(o=>o.kind!=='reference');if(!editable.length)return;const next=editable.some(o=>!o.locked);for(const o of editable)o.locked=next;for(const o of groupMembers(groupId))if(o.kind==='reference')o.locked=true;if(next&&editable.some(o=>o.id===activeId()))document.querySelector('#toolModes button[data-tool="move"]')?.click();forceRender();updateUI();});
      const rename=document.createElement('button');rename.type='button';rename.textContent='✎';rename.title='Rename group';rename.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const value=window.prompt('Group name',groupLabel(groupId));if(value===null)return;const clean=value.trim();if(!clean)return;groupNames.set(groupId,clean);updateUI();setStatus(`${clean} renamed`);});
      header.append(collapse,name,visible,lock,rename);block.append(header,children);first.before(block);for(const row of rows)children.append(row);
    }
...
function syncGroupControls(chosen){const groupButton=document.querySelector('#objectGroupTools [data-group-action="group"]');const ungroupButton=document.querySelector('#objectGroupTools [data-group-action="ungroup"]');if(groupButton)groupButton.disabled=chosen.length<2;if(ungroupButton)ungroupButton.disabled=!chosen.some(o=>o.groupId!=null);}
function updateUI(){cleanSelection();decorateHierarchy();const chosen=selectedObjects(),lockable=chosen.filter(o=>o.kind!=='reference'),editable=chosen.filter(o=>o.kind!=='reference'),linked=chosen.filter(o=>(manager()?.linkedIds?.(o.id)?.length||0)>1);multiButton?.classList.toggle('active',multiEnabled);if(allButton)allButton.textContent=objects().length&&objects().every(o=>selectedIds.has(o.id))?'None':'All';if(visibilityButton)visibilityButton.textContent=chosen.length&&chosen.every(o=>!o.visible)?'Show':'Hide';if(lockButton)lockButton.textContent=lockable.length&&lockable.every(o=>o.locked)?'Unlock':'Lock';if(joinButton)joinButton.disabled=chosen.length<2||chosen.some(o=>o.locked||o.kind==='reference');if(visibilityButton)visibilityButton.disabled=!chosen.length;if(lockButton)lockButton.disabled=!lockable.length;if(clearButton)clearButton.disabled=!chosen.length;if(countLabel)countLabel.textContent=multiEnabled?`${chosen.length} of ${objects().length} selected • active object remains primary`:'Single object selection';if(multiEnabled){if(duplicateButton)duplicateButton.disabled=!chosen.length;if(linkedDuplicateButton)linkedDuplicateButton.disabled=!editable.length;if(makeUniqueButton)makeUniqueButton.disabled=!linked.length;if(renameButton)renameButton.disabled=chosen.length!==1;if(deleteButton)deleteButton.disabled=!chosen.length||chosen.length>=objects().length;}updateRows();syncGroupControls(chosen);}
...
function installCaptureHandlers(){list?.addEventListener('click',e=>{const b=e.target.closest('.outliner-name');if(!b)return;const id=Number(b.closest('.outliner-row')?.dataset.objectId);if(!Number.isFinite(id))return;if(multiEnabled){e.preventDefault();e.stopImmediatePropagation();toggleObjectSelection(id);return;}queueMicrotask(()=>{selectedIds=new Set(activeId()==null?[]:[activeId()]);updateUI();});},true);duplicateButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();duplicateSelection();},true);linkedDuplicateButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();linkedDuplicateSelection();},true);makeUniqueButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();makeUniqueSelection();},true);renameButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;const chosen=[...selectedIds];if(chosen.length!==1){e.preventDefault();e.stopImmediatePropagation();setStatus('Select one object to rename');return;}if(chosen[0]!==activeId())manager()?.activate?.(chosen[0]);},true);deleteButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();deleteSelection();},true);}
