...
function selectGroup(groupId){const members=groupMembers(groupId);ensureGroupPrimary(members);multiEnabled=true;selectedIds=new Set(members.map(o=>o.id));updateUI();forceRender();queueMicrotask(()=>globalThis.__boxlabBooleanUX?.sync?.());setStatus(`${groupLabel(groupId)} • ${members.length} objects selected`);}
