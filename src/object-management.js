...
function injectStyle(){if(document.querySelector('#boxlabObjectManagementStyle'))return;const s=document.createElement('style');s.id='boxlabObjectManagementStyle';s.textContent=`.object-management-tools{display:grid;grid-template-columns:repeat(5,minmax(0,1fr));gap:6px;margin:0 0 8px}.object-management-tools button{min-width:0;padding-left:6px;padding-right:6px}.object-management-tools .active{outline:1px solid currentColor}.object-management-count{grid-column:1/-1;font-size:11px;opacity:.72;padding:0 2px 2px}.outliner-row.object-selected:not(.active){box-shadow:inset 3px 0 0 currentColor}.outliner-row.object-selected .outliner-name{font-weight:600}.boxlab-group-block{position:relative;margin:1px 0 3px;border-radius:6px;background:transparent}.boxlab-group-row{display:grid;grid-template-columns:20px minmax(0,1fr) 26px 28px;gap:2px;align-items:center;min-height:32px;padding:1px 2px;border:1px solid rgba(255,255,255,.08);border-radius:6px;background:rgba(255,255,255,.035)}.boxlab-group-row.group-selected{box-shadow:inset 3px 0 0 #f3b34a;border-color:color-mix(in srgb,#f3b34a 65%,rgba(255,255,255,.25));background:color-mix(in srgb,#f3b34a 12%,rgba(255,255,255,.035))}.boxlab-group-row.group-selected .boxlab-group-name{font-weight:800;color:#f3b34a}.boxlab-group-block.group-context .outliner-row.active{border-color:rgba(255,255,255,.09);background:rgba(255,255,255,.025)}.boxlab-group-block.group-context .outliner-row.object-selected{box-shadow:none}.boxlab-group-block.group-context .outliner-row.object-selected .outliner-name{font-weight:500}.boxlab-group-row button,.boxlab-group-row summary{min-width:0;min-height:28px;margin:0;padding:3px 2px;border:0;border-radius:5px;background:transparent;color:inherit;font-size:11px}.compact-object-row{grid-template-columns:minmax(0,1fr) 28px 28px 30px!important;gap:2px!important;min-height:32px;padding:1px 2px!important;border-radius:6px!important}.compact-object-row button,.compact-object-row summary{min-height:28px!important;margin:0!important;border-radius:5px!important}.compact-object-row .outliner-mini{width:28px!important}.outliner-more,.object-action-more{position:relative;height:28px}.outliner-more>summary,.object-action-more>summary{display:flex;align-items:center;justify-content:center;min-height:28px;padding:2px;list-style:none;cursor:pointer;border-radius:5px}.outliner-more>summary::-webkit-details-marker,.object-action-more>summary::-webkit-details-marker{display:none}.outliner-more-menu,.object-action-menu{position:absolute!important;right:0!important;top:0!important;bottom:auto!important;transform:translateY(calc(-100% - 2px));z-index:80;display:grid;gap:3px;min-width:142px;padding:5px;border:1px solid rgba(255,255,255,.15);border-radius:8px;background:rgba(18,21,27,.98);box-shadow:0 8px 24px rgba(0,0,0,.35)}.outliner-more-menu button,.object-action-menu button{width:100%;min-height:32px!important;margin:0!important;padding:5px 8px!important;text-align:left;background:rgba(255,255,255,.04)}.object-action-more{height:auto}.object-action-more>summary{min-height:36px;border:1px solid rgba(255,255,255,.10);background:rgba(255,255,255,.025)}.object-action-menu{top:0!important;bottom:auto!important;transform:translateY(calc(-100% - 3px))}.outliner-subd-toggle{font-size:10px!important;font-weight:700;opacity:.5}.outliner-subd-toggle.active{opacity:1;outline:1px solid currentColor;background:rgba(255,255,255,.10)}.outliner-subd-toggle:disabled{opacity:.2}.outliner-visibility{font-size:10px!important}.boxlab-group-row button:active,.boxlab-group-row summary:active{background:rgba(255,255,255,.08)}.boxlab-group-row .boxlab-group-name{text-align:left;font-size:12px;font-weight:650;overflow:hidden;text-overflow:ellipsis;white-space:nowrap}.boxlab-group-disclosure{font-size:13px!important;opacity:.78}.boxlab-group-children{display:grid;gap:2px;margin-left:10px;padding:2px 0 1px 10px;border-left:1px solid rgba(255,255,255,.14)}.boxlab-group-children>.outliner-row{margin:0;padding:2px;border-radius:6px;gap:3px}.boxlab-group-children>.outliner-row button{min-height:28px;border-radius:5px}.boxlab-group-more{position:relative;height:28px}.boxlab-group-more>summary{display:flex;align-items:center;justify-content:center;list-style:none;cursor:pointer}.boxlab-group-more>summary::-webkit-details-marker{display:none}.boxlab-group-menu{position:absolute!important;right:0!important;top:0!important;bottom:auto!important;transform:translateY(calc(-100% - 2px));z-index:80;display:grid;gap:3px;min-width:130px;padding:5px;border:1px solid rgba(255,255,255,.15);border-radius:8px;background:rgba(18,21,27,.98);box-shadow:0 8px 24px rgba(0,0,0,.35)}.boxlab-group-menu button{width:100%;text-align:left;padding:5px 8px;background:rgba(255,255,255,.04)}.boxlab-group-block.collapsed .boxlab-group-children{display:none}`;document.head.append(s);}
...
function selectedCompleteGroupIds(){
  const ids=[...new Set(selectedObjects().map(o=>o.groupId).filter(id=>id!=null))];
  return ids.filter(groupId=>{const members=groupMembers(groupId);return members.length>=2&&members.every(o=>selectedIds.has(o.id));});
}
...
function ensureGroupPrimary(members){
  if(!members?.length)return;
  if(members.some(o=>o.id===activeId()))return;
  const target=members.find(o=>o.kind!=='reference'&&!o.locked)||members.find(o=>o.kind!=='reference')||members[0];
  if(target?.id!=null)manager()?.activate?.(target.id,true);
}
function selectGroup(groupId,{additive=false}={}){const members=groupMembers(groupId);if(!members.length)return false;const memberIds=new Set(members.map(o=>o.id)),alreadyComplete=members.every(o=>selectedIds.has(o.id));if(!additive){ensureGroupPrimary(members);selectedIds=new Set(memberIds);}else if(alreadyComplete){for(const id of memberIds)selectedIds.delete(id);}else{for(const id of memberIds)selectedIds.add(id);}multiEnabled=true;updateUI();forceRender();queueMicrotask(()=>globalThis.__boxlabBooleanUX?.sync?.());const complete=selectedCompleteGroupIds();setStatus(complete.length>1?`${complete.length} Groups selected • Boolean ready when both are valid`:`${groupLabel(groupId)} • ${members.length} objects ${additive&&alreadyComplete?'removed from':'selected in'} Multi`);return true;}
function reconcileExistingHierarchy(){
  ...
  for(const block of [...list.querySelectorAll(':scope > .boxlab-group-block')]){
    ...
    if(lock){lock.textContent=editable.length?(allLocked?'Unlock':'Lock'):'Reference • Read Only';lock.title=editable.length?(allLocked?'Unlock editable group members':'Lock editable group members'):'Reference-only group • read-only';lock.disabled=!editable.length;}
  }
}
function decorateHierarchy(){
  ...
  try{
    for(const [groupId,rows] of grouped){
      ...
      const name=document.createElement('button');name.type='button';name.className='boxlab-group-name';name.textContent=groupLabel(groupId);name.title='Select whole group';name.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const complete=selectedCompleteGroupIds(),selected=selectedObjects(),onlyCompleteGroups=selected.length>0&&complete.length>0&&selected.every(o=>o.groupId!=null&&complete.includes(o.groupId));selectGroup(groupId,{additive:multiEnabled&&onlyCompleteGroups});});
      ...
      const editableMembers=members.filter(o=>o.kind!=='reference'),allLocked=editableMembers.length&&editableMembers.every(o=>o.locked);
      const menuLock=document.createElement('button');menuLock.type='button';menuLock.className='boxlab-group-lock';menuLock.textContent=editableMembers.length?(allLocked?'Unlock':'Lock'):'Reference • Read Only';menuLock.disabled=!editableMembers.length;menuLock.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();const editable=groupMembers(groupId).filter(o=>o.kind!=='reference');if(!editable.length)return;globalThis.__boxlabObjectHistory?.checkpoint?.();const next=editable.some(o=>!o.locked);for(const o of editable)o.locked=next;for(const o of groupMembers(groupId))if(o.kind==='reference')o.locked=true;if(next&&editable.some(o=>o.id===activeId()))document.querySelector('#toolModes button[data-tool="move"]')?.click();more.open=false;forceRender();updateUI();});
      ...
      const menuUngroup=document.createElement('button');menuUngroup.type='button';menuUngroup.textContent='Ungroup';menuUngroup.addEventListener('click',e=>{e.preventDefault();e.stopPropagation();more.open=false;selectGroup(groupId);ungroupSelection(new Set(groupMembers(groupId).map(o=>o.id)));});
      menu.append(menuLock,menuRename,menuUngroup);more.append(moreSummary,menu);
      header.append(collapse,name,visible,more);block.append(header,children);first.before(block);for(const row of rows)children.append(row);
    }
  }finally{hierarchyDecorating=false;}
}
function updateRows(){
  const wholeGroupId=selectedWholeGroupId();
  list?.querySelectorAll('.outliner-row').forEach(r=>{const id=Number(r.dataset.objectId),object=objects().find(o=>o.id===id),selected=selectedIds.has(id);r.classList.toggle('object-selected',selected);r.classList.toggle('group-context-member',wholeGroupId!=null&&object?.groupId===wholeGroupId);r.setAttribute('aria-selected',selected?'true':'false');});
  list?.querySelectorAll('.boxlab-group-block').forEach(block=>block.classList.toggle('group-context',wholeGroupId!=null&&Number(block.dataset.groupId)===wholeGroupId));
  list?.querySelectorAll('.boxlab-group-row').forEach(r=>{const groupId=Number(r.parentElement?.dataset?.groupId),members=groupMembers(groupId);r.classList.toggle('group-selected',members.length>0&&members.every(o=>selectedIds.has(o.id)));});
}
...
function cleanupSingletonGroups(){
  ...
  for(const o of objects())if(o.groupId!=null)counts.set(o.groupId,(counts.get(o.groupId)||0)+1);
  for(const o of objects())if(o.groupId!=null&&(counts.get(o.groupId)||0)<2)delete o.groupId;
}
function groupSelection(ids=[...selectedIds]){
  const chosen=objects().filter(o=>ids.includes?.(o.id)||ids instanceof Set&&ids.has(o.id));
  if(chosen.length<2){setStatus('Select at least two objects to group');return false;}
  globalThis.__boxlabObjectHistory?.checkpoint?.();
  const groupId=nextGroupId();
  for(const o of chosen)o.groupId=groupId;
  cleanupSingletonGroups();multiEnabled=true;selectedIds=new Set(chosen.map(o=>o.id));
  updateUI();setStatus(`${chosen.length} objects grouped • ${groupLabel(groupId)}`);return true;
}
function ungroupSelection(ids=[...selectedIds]){
  const idSet=ids instanceof Set?ids:new Set(ids),groups=new Set();
  for(const o of objects())if(idSet.has(o.id)&&o.groupId!=null)groups.add(o.groupId);
  if(!groups.size){setStatus('Selected objects are not grouped');return false;}
  globalThis.__boxlabObjectHistory?.checkpoint?.();
  for(const o of objects())if(groups.has(o.groupId))delete o.groupId;
  cleanupSingletonGroups();pruneGroupMetadata();updateUI();
