...
let selectedIds=new Set(),multiEnabled=false,internalAction=false,toolbar=null,multiButton=null,allButton=null,visibilityButton=null,lockButton=null,clearButton=null,countLabel=null;
...
function buildToolbar(){if(!drawer||toolbar)return;injectStyle();toolbar=document.createElement('div');toolbar.className='object-management-tools';toolbar.id='objectManagementTools';
  ...
  lockButton.addEventListener('click',()=>{const chosen=selectedObjects();if(!chosen.length)return;const lock=chosen.some(o=>!o.locked);for(const o of chosen)o.locked=lock;if(lock&&chosen.some(o=>o.id===activeId()))document.querySelector('#toolModes button[data-tool="move"]')?.click();setStatus(`${chosen.length} object${chosen.length===1?'':'s'} ${lock?'locked':'unlocked'}`);forceRender();updateUI();});
...
function updateRows(){list?.querySelectorAll('.outliner-row').forEach(r=>{const id=Number(r.dataset.objectId);r.classList.toggle('object-selected',selectedIds.has(id));r.setAttribute('aria-selected',selectedIds.has(id)?'true':'false');});}
function syncGroupControls(chosen){
  const groupButton=document.querySelector('#objectGroupTools [data-group-action="group"]');
  const ungroupButton=document.querySelector('#objectGroupTools [data-group-action="ungroup"]');
  if(groupButton)groupButton.disabled=chosen.length<2;
  if(ungroupButton)ungroupButton.disabled=!chosen.some(o=>o.groupId!=null);
}
function updateUI(){cleanSelection();const chosen=selectedObjects();multiButton?.classList.toggle('active',multiEnabled);if(allButton)allButton.textContent=objects().length&&objects().every(o=>selectedIds.has(o.id))?'None':'All';if(visibilityButton)visibilityButton.textContent=chosen.length&&chosen.every(o=>!o.visible)?'Show':'Hide';if(lockButton)lockButton.textContent=chosen.length&&chosen.every(o=>o.locked)?'Unlock':'Lock';if(visibilityButton)visibilityButton.disabled=!chosen.length;if(lockButton)lockButton.disabled=!chosen.length;if(clearButton)clearButton.disabled=!chosen.length;if(countLabel)countLabel.textContent=multiEnabled?`${chosen.length} of ${objects().length} selected • active object remains primary`:'Single object selection';if(multiEnabled){if(duplicateButton)duplicateButton.disabled=!chosen.length;if(renameButton)renameButton.disabled=chosen.length!==1;if(deleteButton)deleteButton.disabled=!chosen.length||chosen.length>=objects().length;}updateRows();syncGroupControls(chosen);}
...
function duplicateSelection(){const m=manager();if(!m)return;const chosen=selectedObjects();if(!chosen.length)return;
  const sourceGroups=new Map();for(const o of chosen)if(o.groupId!=null){if(!sourceGroups.has(o.groupId))sourceGroups.set(o.groupId,nextGroupId()+sourceGroups.size);}
  const snapshots=chosen.map(o=>({name:o.name,mesh:o.mesh.clone(),visible:o.visible,kind:o.kind,origin:o.origin?{...o.origin}:null,groupId:o.groupId,settings:{mirror:{...(o.settings?.mirror||{})},subd:!!o.settings?.subd,subdLevel:Number(o.settings?.subdLevel||1),cage:o.settings?.cage!==false}}));
  const created=[];for(const src of snapshots){const copy=m.addMesh(src.mesh,`${src.name} copy`,{visible:src.visible,locked:false,kind:src.kind,settings:src.settings,enterObjectMode:false});if(!copy)continue;if(src.origin)copy.origin={...src.origin};if(src.groupId!=null&&sourceGroups.has(src.groupId))copy.groupId=sourceGroups.get(src.groupId);created.push(copy.id);}multiEnabled=created.length>1||multiEnabled;selectedIds=new Set(created);setStatus(`${created.length} object${created.length===1?'':'s'} duplicated${sourceGroups.size?' • group relationship preserved':''}`);forceRender();updateUI();}
...
function installCaptureHandlers(){list?.addEventListener('click',e=>{const b=e.target.closest('.outliner-name');if(!b)return;const id=Number(b.closest('.outliner-row')?.dataset.objectId);if(!Number.isFinite(id))return;if(multiEnabled){e.preventDefault();e.stopImmediatePropagation();toggleObjectSelection(id);return;}queueMicrotask(()=>{selectedIds=new Set(activeId()==null?[]:[activeId()]);updateUI();});},true);
  duplicateButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();duplicateSelection();},true);
  renameButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;const chosen=[...selectedIds];if(chosen.length!==1){e.preventDefault();e.stopImmediatePropagation();setStatus('Select one object to rename');return;}if(chosen[0]!==activeId())manager()?.activate?.(chosen[0]);},true);
  deleteButton?.addEventListener('click',e=>{if(internalAction||!multiEnabled)return;e.preventDefault();e.stopImmediatePropagation();deleteSelection();},true);
}
function installObserver(){if(!list)return;new MutationObserver(()=>queueMicrotask(updateUI)).observe(list,{childList:true});}
function initialize(){if(!manager()||!drawer||!list)return false;buildToolbar();selectedIds=new Set(activeId()==null?[]:[activeId()]);installCaptureHandlers();installObserver();updateUI();globalThis.__boxlabObjectSelection={get ids(){return new Set(selectedIds);},get multi(){return multiEnabled;},select(ids=[]){multiEnabled=true;selectedIds=new Set(ids);updateUI();},clear(){selectedIds.clear();updateUI();}};return true;}
