...
function renderMesh(){
  ...
  updateStatus(displayMesh);updateActionAvailability();syncDirectToolControls();syncTransformButtons();syncSelectionModeButtons();syncCreaseControl();
}
...
function addCage(){
  ...
  mesh.edges().forEach((e,index)=>{const geometry=new THREE.BufferGeometry().setFromPoints([mesh.vertices[e.a],mesh.vertices[e.b]]),ga=loopGroupByVertex.get(e.a),gb=loopGroupByVertex.get(e.b),activeGroup=ga!==undefined&&ga===gb?ga:null,isActiveLoop=activeGroup!==null,isCurrentLoop=isActiveLoop&&activeLoopSlides[activeGroup]===activeLoopSlide,isSelectedEdge=selection?.type==='edge'&&selection.index===index,axis=isSelectedEdge?edgeAxis(e.a,e.b):'neutral',mat=isCurrentLoop?currentLoopMaterial:(isActiveLoop?activeLoopMaterial:(isSelectedEdge?axisLineMaterials[axis]:(mesh.edgeCrease(index)>0?creaseEdgeMaterial:edgeMaterial))),line=new THREE.Line(geometry,mat);if(isActiveLoop)line.renderOrder=isCurrentLoop?27:25;if(isSelectedEdge&&!isActiveLoop)line.renderOrder=28;line.userData={kind:'edge',index,loopSlideGroup:activeGroup};root.add(line);if(isSelectedEdge&&!isActiveLoop)addSelectedEdgeOverlay(e.a,e.b,axis);});
...
function updateStatus(displayMesh=mesh){
  const activeMirror=['X','Y','Z'].filter(axis=>mirrorAxes[axis.toLowerCase()]),mirrorText=activeMirror.length?` • Mirror ${activeMirror.join('')}`:'',creaseText=mesh.creases.size?` • ${mesh.creases.size} crease${mesh.creases.size===1?'':'s'}`:'',slideText=activeLoopSlides.length?` • ${activeLoopSlides.length} active loop${activeLoopSlides.length===1?'':'s'}`:'',toolText=directTool?` • ${cap(directTool)}`:'',snapText=axisSnapEnabled&&toolMode==='move'&&!directTool?' • Axis Snap':'',axisText=drag?.kind==='component'&&drag.axisLock&&toolMode==='move'?` • Move ${drag.axisLock.toUpperCase()}`:'';
  ...
  document.querySelector('#selectionStatus').textContent=directTool?`${cap(selectionMode)} mode • ${cap(directTool)} selected`:(selection?`${cap(selectionMode)} ${selection.index+1} selected • ${cap(toolMode)}${drag?.axisLock?` ${drag.axisLock.toUpperCase()}`:''}`:`${cap(selectionMode)} mode • nothing selected`);
}
...
document.querySelectorAll('#selectionModes button').forEach(btn=>btn.addEventListener('click',()=>{setDirectTool(null);selectionMode=btn.dataset.mode;selection=null;selectedEdgeCutT=.5;renderMesh();}));
