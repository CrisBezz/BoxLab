...
let directTool=null;
let selectionModeBeforeDirectTool=null;
...
const axisLineMaterials={
  x:new THREE.LineBasicMaterial({color:0xff4a45,depthTest:false}),
  y:new THREE.LineBasicMaterial({color:0x55d66b,depthTest:false}),
  z:new THREE.LineBasicMaterial({color:0x4f86ff,depthTest:false}),
  neutral:new THREE.LineBasicMaterial({color:0xffffff,depthTest:false})
};
const axisOverlayMaterials={
  x:new THREE.MeshBasicMaterial({color:0xff4a45,depthTest:false}),
  y:new THREE.MeshBasicMaterial({color:0x55d66b,depthTest:false}),
  z:new THREE.MeshBasicMaterial({color:0x4f86ff,depthTest:false}),
  neutral:new THREE.MeshBasicMaterial({color:0xffffff,depthTest:false})
};
...
function syncSelectionModeButtons(){document.querySelectorAll('#selectionModes button').forEach(b=>b.classList.toggle('active',!directTool&&b.dataset.mode===selectionMode));}
function setDirectTool(tool){
  const next=tool||null;
  if(next&&!directTool){selectionModeBeforeDirectTool=selectionMode;selection=null;}
  directTool=next;
  if(!directTool&&selectionModeBeforeDirectTool){selectionMode=selectionModeBeforeDirectTool;selectionModeBeforeDirectTool=null;}
  syncSelectionModeButtons();syncDirectToolControls();updateStatus();
}
...
function renderMesh(){
  ...
  updateStatus(displayMesh);updateActionAvailability();syncDirectToolControls();syncCreaseControl();
}
...
function addSelectedEdgeOverlay(a,b,axis){const start=mesh.vertices[a],end=mesh.vertices[b];if(!start||!end)return;const delta=new THREE.Vector3().subVectors(end,start),length=delta.length();if(length<1e-6)return;const geometry=new THREE.CylinderGeometry(.022,.022,length,10,1,false);const overlay=new THREE.Mesh(geometry,axisOverlayMaterials[axis]||axisOverlayMaterials.neutral);overlay.position.copy(start).add(end).multiplyScalar(.5);overlay.quaternion.setFromUnitVectors(new THREE.Vector3(0,1,0),delta.normalize());overlay.renderOrder=30;overlay.userData.kind='edge-selection-overlay';root.add(overlay);}
function addCage(){
  ...
  mesh.edges().forEach((e,index)=>{const geometry=new THREE.BufferGeometry().setFromPoints([mesh.vertices[e.a],mesh.vertices[e.b]]);const ga=loopGroupByVertex.get(e.a),gb=loopGroupByVertex.get(e.b);const activeGroup=ga!==undefined&&ga===gb?ga:null;const isActiveLoop=activeGroup!==null;const isCurrentLoop=isActiveLoop&&activeLoopSlides[activeGroup]===activeLoopSlide;const isSelectedEdge=selection?.type==='edge'&&selection.index===index;const axis=isSelectedEdge?edgeAxis(e.a,e.b):'neutral';const mat=isCurrentLoop?currentLoopMaterial:(isActiveLoop?activeLoopMaterial:(isSelectedEdge?axisLineMaterials[axis]:(mesh.edgeCrease(index)>0?creaseEdgeMaterial:edgeMaterial)));const line=new THREE.Line(geometry,mat);if(isActiveLoop)line.renderOrder=isCurrentLoop?27:25;if(isSelectedEdge&&!isActiveLoop)line.renderOrder=28;line.userData={kind:'edge',index,loopSlideGroup:activeGroup};root.add(line);if(isSelectedEdge&&!isActiveLoop)addSelectedEdgeOverlay(e.a,e.b,axis);});
  mesh.vertices.forEach((v,index)=>{const selected=selection?.type==='vertex'&&selection.index===index;const geometry=new THREE.SphereGeometry(selected?0.0375:0.0275,12,8);const dot=new THREE.Mesh(geometry,selected?selectedVertexMaterial:vertexMaterial);dot.position.copy(v);dot.userData={kind:'vertex',index};root.add(dot);});
...
function updateStatus(displayMesh=mesh){
  const activeMirror=['X','Y','Z'].filter(axis=>mirrorAxes[axis.toLowerCase()]);const mirrorText=activeMirror.length?` • Mirror ${activeMirror.join('')}`:'';const creaseText=mesh.creases.size?` • ${mesh.creases.size} crease${mesh.creases.size===1?'':'s'}`:'';const slideText=activeLoopSlides.length?` • ${activeLoopSlides.length} active loop${activeLoopSlides.length===1?'':'s'}`:'';const toolText=directTool?` • ${cap(directTool)}`:'';
  ...
  document.querySelector('#selectionStatus').textContent=directTool?`${cap(directTool)} selected`:(selection?`${cap(selectionMode)} ${selection.index+1} selected • ${cap(toolMode)}`:`${cap(selectionMode)} mode • nothing selected`);
}
function updateActionAvailability(){const faceSelected=selection?.type==='face'&&!!mesh.faces[selection.index];const edgeSelected=selection?.type==='edge'&&!!mesh.edges()[selection.index];document.querySelector('#extrudeBtn').disabled=false;document.querySelector('#insetBtn').disabled=false;document.querySelector('#deleteFaceBtn').disabled=!faceSelected;document.querySelector('#loopCutBtn').disabled=false;document.querySelector('#loopSlide').disabled=!activeLoopSlide;document.querySelector('#applyCreaseBtn').disabled=!edgeSelected;document.querySelector('#clearCreaseBtn').disabled=!edgeSelected||mesh.edgeCrease(selection?.index)<0.001;}
function syncDirectToolControls(){
  const count=Number(document.querySelector('#loopCutCount')?.value||1);const loop=document.querySelector('#loopCutBtn'),extrude=document.querySelector('#extrudeBtn'),inset=document.querySelector('#insetBtn');
  loop?.classList.toggle('active',directTool==='loopCut');extrude?.classList.toggle('active',directTool==='extrude');inset?.classList.toggle('active',directTool==='inset');
  if(loop)loop.textContent=count===1?'Loop Cut':`Loop Cut ×${count}`;
  if(extrude)extrude.textContent=directTool==='extrude'&&drag?.kind==='faceTool'&&drag.armed?`Extrude ${drag.liveValue>=0?'+':''}${drag.liveValue.toFixed(2)}`:'Extrude';
  if(inset)inset.textContent=directTool==='inset'&&drag?.kind==='faceTool'&&drag.armed?`Inset ${Math.round(drag.liveValue*100)}%`:'Inset';
}
function syncCreaseControl(){if(selection?.type!=='edge')return;const current=mesh.edgeCrease(selection.index);const pct=current>0?Math.round(current*100):100;document.querySelector('#creaseStrength').value=String(pct);document.querySelector('#creaseStrengthOut').textContent=`${pct}%`;}

const cap=s=>s.charAt(0).toUpperCase()+s.slice(1);const sameSelection=(a,b)=>!!a&&!!b&&a.type===b.type&&a.index===b.index;
...
function pick(event){const kind=selectionMode==='vertex'?'vertex':selectionMode==='edge'?'edge':'face';return pickKind(event,kind);}
function edgeTapFraction(edgeIndex,event){const edge=mesh.edges()[edgeIndex];if(!edge)return .5;const a=worldToScreen(mesh.vertices[edge.a]),b=worldToScreen(mesh.vertices[edge.b]);const ab=b.clone().sub(a),lenSq=ab.lengthSq();if(lenSq<1)return .5;const p=new THREE.Vector2(event.clientX,event.clientY);return THREE.MathUtils.clamp(p.sub(a).dot(ab)/lenSq,.05,.95);}
function pickLoopSlide(event){if(!activeLoopSlides.length)return null;setPointer(event);raycaster.setFromCamera(pointer,camera);const hits=raycaster.intersectObjects(root.children.filter(o=>Number.isInteger(o.userData.loopSlideGroup)),false);if(!hits.length)return null;const line=hits[0].object,groupIndex=line.userData.loopSlideGroup,group=activeLoopSlides[groupIndex];const edge=mesh.edges()[line.userData.index];if(!group||!edge)return null;const candidates=group.filter(item=>item.vertex===edge.a||item.vertex===edge.b);if(!candidates.length)return null;const p=new THREE.Vector2(event.clientX,event.clientY);const item=candidates.sort((u,v)=>worldToScreen(mesh.vertices[u.vertex]).distanceToSquared(p)-worldToScreen(mesh.vertices[v.vertex]).distanceToSquared(p))[0];return{item,group,groupIndex};}
...
function doUndo(){const previous=history.undo(mesh);if(!previous)return false;mesh=previous;selection=null;selectedEdgeCutT=.5;clearLoopSlide();setDirectTool(null);renderMesh();return true;}
function doRedo(){const next=history.redo(mesh);if(!next)return false;mesh=next;selection=null;selectedEdgeCutT=.5;clearLoopSlide();setDirectTool(null);renderMesh();return true;}
...
canvas.addEventListener('pointermove',event=>{
  ...
  if(drag.kind==='faceTool'){
    mesh=drag.startMesh.clone();
    if(drag.faceTool==='extrude'){const faceIndex=drag.faceIndex,normal=drag.startMesh.faceNormal(faceIndex),center=drag.startMesh.faceCenter(faceIndex),facing=Math.max(.2,Math.abs(normal.dot(new THREE.Vector3().subVectors(camera.position,center).normalize()))),signedPixels=new THREE.Vector2(dx,dy).dot(drag.normal2D),amount=signedPixels*.006/facing;drag.liveValue=amount;selection=mesh.extrudeFace(faceIndex,amount);}
    else{const amount=THREE.MathUtils.clamp(new THREE.Vector2(dx,dy).dot(drag.insetRail)/Math.max(30,drag.insetScale),.01,.95);drag.liveValue=amount;selection=mesh.insetFace(drag.faceIndex,amount);}
    renderMesh();return;
  }
  const now=rayPlanePoint(event,drag.plane);if(!now||!drag.last||!drag.start)return;beginDragChange();mesh=drag.startMesh.clone();const total=now.clone().sub(drag.start);if(toolMode==='move')mesh.moveComponent(drag.selection,total);else if(toolMode==='scale'){const factor=THREE.MathUtils.clamp(Math.exp((dx-dy)*.006),.1,5);mesh.scaleComponent(drag.selection,factor);}drag.last=now;renderMesh();
});
function endDrag(event){if(!drag||drag.pointerId!==event.pointerId)return;const finishedFaceTool=drag.kind==='faceTool'&&drag.armed;drag=null;controls.enabled=true;if(finishedFaceTool){setDirectTool(null);renderMesh();}}
canvas.addEventListener('pointerup',endDrag);canvas.addEventListener('pointercancel',endDrag);

document.querySelectorAll('#selectionModes button').forEach(btn=>btn.addEventListener('click',()=>{setDirectTool(null);document.querySelectorAll('#selectionModes button').forEach(b=>b.classList.remove('active'));btn.classList.add('active');selectionMode=btn.dataset.mode;selection=null;selectedEdgeCutT=.5;renderMesh();}));
document.querySelectorAll('#toolModes button').forEach(btn=>btn.addEventListener('click',()=>{setDirectTool(null);document.querySelectorAll('#toolModes button').forEach(b=>b.classList.remove('active'));btn.classList.add('active');toolMode=btn.dataset.tool;updateStatus();}));
...
window.addEventListener('resize',resize);function animate(){requestAnimationFrame(animate);controls.update();renderer.render(scene,camera);}resize();renderMesh();animate();