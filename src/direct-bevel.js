...
canvas?.addEventListener('pointerdown',e=>{if(!armed||!e.isPrimary)return;const i=hit(e),mesh=state()?.mesh;if(!Number.isInteger(i)||!mesh)return;e.preventDefault();e.stopImmediatePropagation();const existing=selectedEdgeIds(),multiCandidate=!!multiToggle?.checked&&existing.length>1&&existing.includes(i)?existing:null,multiValid=multiCandidate?mesh.generalBevelSelectionInfo?.(multiCandidate):null,ids=multiValid?[...multiCandidate]:[i];if(!multiValid)bridge()?.set?.('edge',[i]);const valid=multiValid||mesh.generalBevelSelectionInfo?.(ids);if(!valid){document.querySelector('#selectionStatus').textContent=ids.length>1?(mesh.__lastBevelError||'Selected edges cannot be bevelled together'):(mesh.__lastBevelError||'This edge cannot be bevelled');return;}drag={id:e.pointerId,x:e.clientX,width:Number(width.value||20),mesh,before:mesh.clone(),ids:[...valid.ids],mode:valid.mode,preview:false};canvas.setPointerCapture?.(e.pointerId)},true);
...
globalThis.__boxlabDirectBevel={version:'0.36.18.460',get active(){return armed;},applyExact};
