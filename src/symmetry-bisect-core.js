...
export function bisectMesh(mesh,{axis='x',keep='positive'}={}){
  if(!mesh?.vertices?.length||!mesh?.faces?.length)return{ok:false,reason:'empty-mesh'};
  const ai=axisIndex(axis),sign=keep==='negative'?-1:1;
  ...
  const getOriginal=index=>{
    ...
    if(!v)return null;
    if(Math.abs(coord(v,ai))<=EPS)setCoord(v,ai,0);
  ...
  const getIntersection=(a,b)=>{
    ...
    const ca=coord(va,ai),cb=coord(vb,ai),den=cb-ca;
    if(Math.abs(den)<=EPS)return null;
    const t=-ca/den;
    const v=va.clone().lerp(vb,t);setCoord(v,ai,0);
    const out=vertices.length;vertices.push(v);intersectionMap.set(key,out);return out;
  };
  const inside=index=>sign*coord(mesh.vertices[index],ai)>=-EPS;
  ...
  return{ok:true,mesh:result,axis,keep,cutVertices:intersectionMap.size};
}

export function symmetryBisect(mesh,{axis='x',keep='positive',mirror=true}={}){
  const cut=bisectMesh(mesh,{axis,keep});
  if(!cut.ok)return cut;
  if(!mirror)return{...cut,mirrored:false};
  const axes={x:false,y:false,z:false};axes[axis]=true;
  const mirrored=applyMirror(cut.mesh,axes);
  return{ok:true,mesh:mirrored,axis,keep,mirrored:true,cutVertices:cut.cutVertices};
}
