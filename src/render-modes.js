...
function setMode(next){
  ...
  if(status)status.textContent=`View • ${mode==='wire'?'Wire + Solid':mode==='matcap'?'MatCap':mode[0].toUpperCase()+mode.slice(1)}`;
}
...
function installUI(){
  ...
  wrap.innerHTML='<button type="button" data-render="solid" class="active">Solid</button><button type="button" data-render="clay">Clay</button><button type="button" data-render="matcap">MatCap</button><button type="button" data-render="wire">Wire + Solid</button><button type="button" data-render="xray">X-Ray</button>';
  host.append(wrap);
...
@media(max-width:1100px){#renderModes button{padding:5px 7px}#renderModes{max-width:42vw;overflow-x:auto;scrollbar-width:none}#renderModes::-webkit-scrollbar{display:none}}
`;
