...
import {applyFaceGroupColours} from './facegroup-colours-core.js?v=0.36.18.452';
...
function sourceMeshForBody(body){
  ...
  if(body?.userData?.kind==='boxlab-inactive-body'&&m)return m.objects?.find(item=>item.id===body.userData.objectId)?.mesh||null;
  return null;
}
...
function frontOnly(material){if(!material?.clone)return material;let front=frontMaterialCache.get(material);if(!front){front=material.clone();front.side=THREE.FrontSide;front.needsUpdate=true;frontMaterialCache.set(material,front);}return front;}
...
function refreshStudio(){
  if(mode!=='studio'||studioRefreshQueued)return;studioRefreshQueued=true;
  requestAnimationFrame(()=>{studioRefreshQueued=false;const state=bridge(),scene=state?.scene;if(!scene||!studioFloor||!studioKey)return;const bounds=new THREE.Box3();let hasVisibleBody=false;scene.traverse(object=>{const kind=object?.userData?.kind;if(!object?.isMesh||!object.visible||(kind!=='body'&&kind!=='boxlab-inactive-body'))return;bounds.expandByObject(object);hasVisibleBody=true;object.castShadow=true;object.receiveShadow=true;});if(!hasVisibleBody||bounds.isEmpty())return;const center=bounds.getCenter(new THREE.Vector3()),size=bounds.getSize(new THREE.Vector3()),span=Math.max(size.x,size.z,1),height=Math.max(size.y,1),radius=Math.max(span,height)*.5,floorSize=Math.max(span*2.8,24);studioFloor.position.set(center.x,bounds.min.y-.025,center.z);studioFloor.scale.set(floorSize,floorSize,1);studioKeyTarget.position.copy(center);const lightDistance=Math.max(radius*3.5,12);const lightAngle=globalThis.__boxlabStudioLightAngle?.radians||0;const elevation=globalThis.__boxlabStudioLightAngle?.elevationRadians??(52*Math.PI/180);const horizontal=Math.cos(elevation),vertical=Math.sin(elevation);const baseLength=Math.hypot(.55,.45),baseX=.55/baseLength,baseZ=.45/baseLength,cos=Math.cos(lightAngle),sin=Math.sin(lightAngle);const lightX=(baseX*cos-baseZ*sin)*horizontal,lightZ=(baseX*sin+baseZ*cos)*horizontal;studioKey.position.set(center.x+lightDistance*lightX,center.y+lightDistance*vertical,center.z+lightDistance*lightZ);studioKey.intensity=2.15*(globalThis.__boxlabStudioLightAngle?.intensityScale??1);const shadowHalf=Math.max(radius*1.45,6),camera=studioKey.shadow.camera;camera.left=-shadowHalf;camera.right=shadowHalf;camera.top=shadowHalf;camera.bottom=-shadowHalf;camera.near=.1;camera.far=Math.max(lightDistance*3,40);camera.updateProjectionMatrix();studioKey.shadow.needsUpdate=true;});
}
function syncStudio(){const state=bridge(),scene=state?.scene,renderer=state?.renderer;if(!scene||!renderer)return;ensureStudioRig();const enabled=mode==='studio';if(studioRig)studioRig.visible=enabled;renderer.shadowMap.enabled=enabled;if(enabled){renderer.shadowMap.type=THREE.PCFSoftShadowMap;renderer.shadowMap.autoUpdate=true;}if(state.baseHemisphere)state.baseHemisphere.intensity=enabled?.52:1.2;if(state.key)state.key.intensity=enabled?1.35:3.2;if(enabled){const backdrop=globalThis.__boxlabBackdropPresets?.colour;scene.background.copy(new THREE.Color(Number.isFinite(backdrop)?backdrop:0x131924));}else scene.background.copy(originalBackground||new THREE.Color(0x111318));if(enabled)refreshStudio();}
...
function applyMode(body){
  ...
  if(mode==='studio'){body.material=inactive?studioInactiveMaterial:studioMaterial;addBackface(body);}
    ...
    const result=applyFaceGroupColours(body.geometry,sourceMeshForBody(body));
    body.material=inactive?facegroupInactiveMaterial:facegroupMaterial;
    addBackface(body);body.userData.boxlabFacegroupCount=result?.groups||0;
...
function setMode(next){mode=next;document.querySelectorAll('#viewportRenderLooks button[data-render]').forEach(button=>button.classList.toggle('active',button.dataset.render===mode));if(!applyToSceneBodies())rebuild();document.dispatchEvent(new CustomEvent('boxlab-render-mode-change',{detail:{mode}}));if(status)status.textContent=`View • ${modeLabel()}`;}
...
function installUI(){const host=document.querySelector('#viewportRenderLooks');if(!host||host.dataset.ready==='true')return false;host.dataset.ready='true';host.innerHTML='<button type="button" data-render="studio" class="active">Studio</button><button type="button" data-render="solid">Solid</button><button type="button" data-render="clay">Clay</button><button type="button" data-render="matcap">MatCap</button><button type="button" data-render="facegroups">Facegroups</button><button type="button" data-render="normals">Normals</button><button type="button" data-render="wire">Wire</button><button type="button" data-render="xray">X-Ray</button>';host.addEventListener('click',event=>{const button=event.target.closest('button[data-render]');if(!button)return;event.preventDefault();event.stopPropagation();setMode(button.dataset.render);});return true;}
if(!installUI())queueMicrotask(installUI);queueMicrotask(rebuild);
