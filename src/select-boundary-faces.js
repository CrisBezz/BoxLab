// BoxLab v0.36.18.61 — non-destructive boundary Face inspection.
// Selects faces touching at least one open boundary edge (exactly one real face user).
...
button.id='selectBoundaryFacesBtn';
button.type='button';
...
function place(){
  ...
  if(existing){
    existing.style.gridTemplateColumns='repeat(4,minmax(0,1fr))';
    if(button.parentElement!==existing)existing.appendChild(button);
    for(const id of ['selectDegenerateFacesBtn','selectDuplicateFacesBtn','selectIsolatedFacesBtn']){
...
function inspect(m){
  if(!m)return{indices:[],count:0,boundaryEdges:0};
  const faces=m.faces||[];
  ...
  return{indices,count:indices.length,boundaryEdges};
}
...
function apply(){
  ...
  queueMicrotask(()=>{
    ...
    if(status)status.textContent=info.count
      ?`Boundary Faces • ${info.count} face${info.count===1?'':'s'} selected • ${info.boundaryEdges} open edge${info.boundaryEdges===1?'':'s'}`
      :'Topology check • 0 boundary faces';
  });
}
...
function sync(){
  ...
  button.title=info.count
    ?`Select ${info.count} face${info.count===1?'':'s'} touching ${info.boundaryEdges} open boundary edge${info.boundaryEdges===1?'':'s'}`
    :'No open boundary faces found';
}
...
globalThis.__boxlabSelectBoundaryFaces={version:'0.36.18.61',inspect,apply};
