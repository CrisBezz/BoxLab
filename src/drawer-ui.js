...
import('./object-drawer-retain.js?v=0.36.11.3').catch(error => console.warn('BoxLab object drawer retain failed to load', error));
import('./studio-scene-fix.js?v=0.36.8.1').catch(error => console.warn('BoxLab Studio scene fix failed to load', error));
import('./extrude-through-viability.js?v=0.36.11.3').catch(error => console.warn('BoxLab Extrude Through viability guard failed to load', error));
import('./object-origin.js?v=0.36.4.0-recovery1').then(() => {
  installGroupUiPolish();
  syncDrawerToMode();
  const version = document.querySelector('#appVersion');
  if (version) version.textContent = 'v0.36.11.3';
  document.title = 'BoxLab v0.36.11.3';
