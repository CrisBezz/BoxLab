...
import('./extrude-corner-through.js?v=0.36.14.12').catch(error => console.warn('BoxLab corner Through handler failed to load', error));
import('./corner-through-cleanup.js?v=0.36.14.13').catch(error => console.warn('BoxLab corner Through cleanup failed to load', error));
import('./object-origin.js?v=0.36.4.0-recovery1').then(() => {
  installGroupUiPolish();
  syncDrawerToMode();
  const version = document.querySelector('#appVersion');
  if (version) version.textContent = 'v0.36.14.13';
  document.title = 'BoxLab v0.36.14.13';
