...
function hitFace(event,s,ids){const r=canvas.getBoundingClientRect();pointer.set((event.clientX-r.left)/r.width*2-1,-((event.clientY-r.top)/r.height*2-1));raycaster.setFromCamera(pointer,s.camera);let best=null;for(const fi of ids){const f=s.mesh?.faces?.[fi];if(!f||f.length<3)continue;const pos=[];for(let i=1;i<f.length-1;i++)for(const vi of[f[0],f[i],f[i+1]]){const v=s.mesh.vertices[vi];pos.push(v.x,v.y,v.z);}const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(pos,3));const m=new THREE.MeshBasicMaterial({side:THREE.DoubleSide});const obj=new THREE.Mesh(g,m),hit=raycaster.intersectObject(obj,false)[0];g.dispose();m.dispose();if(hit&&(!best||hit.distance<best.distance))best={index:fi,distance:hit.distance};}return best?.index??null;}
...
canvas?.addEventListener('pointerdown',e=>{if(!e.isPrimary||directToolActive())return;const index=hitSelected(e);if(!Number.isInteger(index))return;press={id:e.pointerId,index,mode:mode(),x:e.clientX,y:e.clientY,moved:false};},true);
...
canvas?.addEventListener('pointerup',e=>{if(!press||press.id!==e.pointerId)return;const p=press;press=null;if(p.moved||Math.hypot(e.clientX-p.x,e.clientY-p.y)>=TAP)return;setTimeout(()=>{const b=bridge();if(!b||b.mode?.()!==p.mode)return;const ids=[...new Set(b.indices?.()||[])];if(!ids.includes(p.index))return;b.set(p.mode,ids.filter(i=>i!==p.index));},0);},true);
