...
import {planThrough,validateThrough} from './through-kernel.js?v=0.36.16.0';
...
window.addEventListener('pointerup',event=>{if(internalCancel)return;if(takeover&&takeover.id===event.pointerId){event.preventDefault();event.stopImmediatePropagation();const t=takeover;takeover=null;probe=null;if(t.ready){const built=buildRegion(t.before,t.plan),valid=built?validateThrough(built):{ok:false,reason:'build-failed'};if(built&&valid.ok){restore(t.m,built);bridge()?.set?.('face',[]);if(status)status.textContent='Extrude Through • sequential topology fallback';}else{restore(t.m,t.before);bridge()?.set?.('face',[t.faceIndex]);if(status)status.textContent=`Extrude Through • fallback rollback • ${valid.reason||'validation failed'}`;}}else{previewExtrude(t.m,t.before,t.faceIndex,t.distance);bridge()?.set?.('face',[t.faceIndex]);}render();return;}probe=null;},true);
