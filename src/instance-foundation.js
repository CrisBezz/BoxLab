// BoxLab v0.36.19.4 — linked instance editing foundation.
// World-space object meshes remain authoritative for placement. Placement is
// captured explicitly when leaving Object mode. Linked component edits are
// committed only from canvas gestures, then normalized from the shared source.
...
function restoreMesh(target,source){if(!target||!source)return;target.vertices=source.vertices.map(v=>v.clone());target.faces=source.faces.map(f=>[...f]);target.creases=new Map(source.creases||[]);target.looseEdges=new Set(source.looseEdges||[]);target.looseVertices=new Set(source.looseVertices||[]);target.edges?.();}
...
function commitSharedEdit(){const m=manager(),live=state()?.mesh,o=activeObject();if(!m||!live||!o)return false;const source=ensureSource(o);if(!source||linkedCount(o.sourceId)<2){o.mesh=live.clone();return false;}const inverse=matrixFor(o).invert(),local=transformMesh(live,inverse);if(meshNear(local,source.mesh)){o.mesh=live.clone();return false;}source.mesh=local.clone();source.revision++;
  for(const peer of m.objects){if(peer.sourceId!==o.sourceId)continue;peer.mesh=transformMesh(source.mesh,matrixFor(peer));}
  // Rebuild the active live mesh from the same committed source so repeated
  // Extrude/Inset starts from a fresh EditableMesh topology, not preview caches.
  const normalized=transformMesh(source.mesh,matrixFor(o));
  restoreMesh(live,normalized);
  o.mesh=normalized.clone();
  m.saveActive?.();
  return true;
}
...
function queueGestureSync(releaseMode){if(syncQueued)return;syncQueued=true;requestAnimationFrame(()=>{syncQueued=false;syncAfterGesture(releaseMode);});}
...
function initialize(){if(!manager()||!state()?.mesh)return false;ensureAll();installUI();decorate();updateButtons();globalThis.__boxlabObjectGeometry={version:'0.36.19.4',evaluatedMesh(id){const o=objects().find(x=>x.id===id);return o?evaluatedMesh(o):null;},sourceId(id){return objects().find(x=>x.id===id)?.sourceId||null;},linkedIds(id){const o=objects().find(x=>x.id===id);return o?objects().filter(x=>x.sourceId===o.sourceId).map(x=>x.id):[];}};
  ...
  // Only modelling gestures on the viewport are eligible. UI pointer-ups must
  // never become geometry commits.
  window.addEventListener('pointerup',event=>{if(event.target===canvas&&mode()!=='object')queueGestureSync(mode());},true);
