// BoxLab v0.36.19.1 — stable linked instance foundation.
// Keeps BoxLab's existing world-space object meshes as the authoritative placement.
// Shared source geometry is updated only in component edit modes; object transforms
// merely update the per-instance placement matrix and never rewrite the live mesh.
...
function captureActivePlacement(){const m=manager(),live=state()?.mesh,o=activeObject();if(!m||!live||!o)return;const source=ensureSource(o);if(!source||live.vertices.length!==source.mesh.vertices.length)return;const placement=derivePlacement(source.mesh,live);if(placement)setMatrix(o,placement);o.mesh=live.clone();}
function syncActiveEdit(){const m=manager(),live=state()?.mesh,o=activeObject();if(!m||!live||!o)return false;const source=ensureSource(o);if(!source)return false;const inv=matrixFor(o).invert();const local=transformMesh(live,inv);if(meshNear(local,source.mesh)){o.mesh=live.clone();return false;}source.mesh=local.clone();source.revision++;for(const peer of m.objects){if(peer.sourceId!==o.sourceId)continue;peer.mesh=evaluatedMesh(peer);}o.mesh=live.clone();return true;}
function refreshPeers(){document.querySelector('#cageToggle')?.dispatchEvent(new Event('change',{bubbles:true}));}
function syncNow(){if(syncing)return;syncing=true;try{ensureAll();if(mode()==='object')captureActivePlacement();else if(syncActiveEdit())requestAnimationFrame(refreshPeers);decorate();updateButtons();}finally{syncing=false;}}

function linkedDuplicate(){const m=manager(),src=activeObject();if(!m||!src)return;m.saveActive?.();captureActivePlacement();const source=ensureSource(src);if(!source)return;const copy=m.addMesh(src.mesh.clone(),`${src.name} linked`,{settings:src.settings,visible:src.visible!==false,locked:false,kind:src.kind,enterObjectMode:true});if(!copy)return;copy.sourceId=src.sourceId;setMatrix(copy,matrixFor(src));copy.mesh=src.mesh.clone();if(src.origin)copy.origin={...src.origin};requestAnimationFrame(()=>{decorate();updateButtons();});if(status)status.textContent=`Linked instance created • ${linkedCount(src.sourceId)} share geometry`;}
function makeUnique(){const o=activeObject();if(!o)return;captureActivePlacement();const source=ensureSource(o);if(!source||linkedCount(o.sourceId)<2){if(status)status.textContent='Object is already unique';return;}o.sourceId=newSource(source.mesh);decorate();updateButtons();if(status)status.textContent=`${o.name} made unique`;}
function installUI(){if(!drawer||document.querySelector('#instanceFoundationTools'))return;const row=document.createElement('div');row.id='instanceFoundationTools';row.className='outliner-actions';row.style.gridTemplateColumns='1fr 1fr';linkedButton=document.createElement('button');linkedButton.type='button';linkedButton.textContent='Linked Duplicate';uniqueButton=document.createElement('button');uniqueButton.type='button';uniqueButton.textContent='Make Unique';row.append(linkedButton,uniqueButton);const standard=document.querySelector('#outlinerAddBtn')?.parentElement;standard?.before(row)??drawer.append(row);linkedButton.addEventListener('click',linkedDuplicate);uniqueButton.addEventListener('click',makeUnique);}
...
function initialize(){if(!manager()||!state()?.mesh)return false;ensureAll();installUI();syncNow();globalThis.__boxlabObjectGeometry={version:'0.36.19.1',evaluatedMesh(id){const o=objects().find(x=>x.id===id);return o?evaluatedMesh(o):null;},sourceId(id){return objects().find(x=>x.id===id)?.sourceId||null;},linkedIds(id){const o=objects().find(x=>x.id===id);return o?objects().filter(x=>x.sourceId===o.sourceId).map(x=>x.id):[];}};window.addEventListener('boxlab-bridge-state',syncNow);document.querySelector('#selectionModes')?.addEventListener('click',()=>queueMicrotask(syncNow),true);list?.addEventListener('click',()=>requestAnimationFrame(syncNow),true);window.dispatchEvent(new Event('boxlab-instance-foundation-ready'));return true;}
