// BoxLab v0.36.18.273 — SubD-friendly unequal open-chain Bridge.
// Conservatively densifies the smaller open chain, then builds an all-quad strip.
// If the all-quad route is not safe, the proven v266 open-chain solver remains authoritative.

const VERSION='0.36.18.273';
const EPS=1e-12;
const MAX_ADDED=4;
const MAX_RATIO=2;
...
function quadValid(mesh,face){
  if(!Array.isArray(face)||face.length!==4||new Set(face).size!==4)return false;
...
function validateCandidate(mesh,faces,chainA,chainB){
  const aSet=new Set(chainA),bSet=new Set(chainB),scale=chainScale(mesh,chainA,chainB),limit=Math.max(scale*36,1e-8);let connectors=0;
  for(const face of faces){
    if(!quadValid(mesh,face))return false;
    for(let i=0;i<4;i++){
      const a=face[i],b=face[(i+1)%4],cross=(aSet.has(a)&&bSet.has(b))||(bSet.has(a)&&aSet.has(b));if(!cross)continue;
      const d2=mesh.vertices[a]?.distanceToSquared?.(mesh.vertices[b]);if(!Number.isFinite(d2)||d2>limit)return false;connectors++;
    }
  }
  return connectors>0;
}
  ...
  for(const reverse of[false,true]){
    ...
    for(const flip of[false,true]){
      ...
      for(let i=0;i<chainA.length-1;i++){
        ...
        if(!quadValid(mesh,q)){valid=false;break;}faces.push(q);
      }
      if(!valid||!validateCandidate(mesh,faces,chainA,mapped))continue;
      ...
      if(!best||score<best.score)best={faces,mapped,reverse,flip,windingPenalty:winding,surfacePenalty:surface,score};
    }
  }
  return best;
}
...
function windingValidation(mesh){
  const uses=new Map();
  for(let fi=0;fi<mesh.faces.length;fi++){
    const face=mesh.faces[fi];if(!Array.isArray(face)||face.length<3||new Set(face).size!==face.length)return false;
    for(let i=0;i<face.length;i++){
      const a=face[i],b=face[(i+1)%face.length],key=a<b?`${a}:${b}`:`${b}:${a}`,dir=a<b?1:-1;
      if(!uses.has(key))uses.set(key,[]);uses.get(key).push(dir);
    }
  }
  for(const owners of uses.values())if(owners.length>2||(owners.length===2&&owners[0]===owners[1]))return false;
  return true;
}
...
export function installOpenChainAllQuadBridge(EditableMesh){
  const proto=EditableMesh?.prototype;if(!proto||proto.__openChainAllQuadBridge273Installed)return;
  ...
  proto.bridgeSelectedEdges=function(edgeIndices){
    ...
    const before=topology.validateTopology(this,{allowBoundary:true});if(!before.ok)return baseSelected.call(this,edgeIndices);
    ...
    if(!denseA||!denseB)return baseSelected.call(this,edgeIndices);
    const plan=bestEqualQuadPlan(trial,denseA,denseB);if(!plan)return baseSelected.call(this,edgeIndices);
    ...
    if(!validation.ok||!windingValidation(trial)||!plan.faces.every(f=>quadValid(trial,f)))return baseSelected.call(this,edgeIndices);
    ...
    globalThis.__boxlabOpenChainAllQuadBridge={version:VERSION,ok:true,allQuad:true,addedVertices,denseCounts:result.denseCounts};
    return result;
  };

  proto.__openChainAllQuadBridge273Installed=true;
  globalThis.__boxlabOpenChainAllQuadBridge={version:VERSION,ok:null,allQuad:false,addedVertices:0,denseCounts:[]};
}
