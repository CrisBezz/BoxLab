...
function hitFace(event,m,camera,allowed=null){setPointer(event);raycaster.setFromCamera(pointer,camera);const pickers=[];const allow=allowed?new Set(allowed):null;for(let faceIndex=0;faceIndex<m.faces.length;faceIndex++){if(allow&&!allow.has(faceIndex))continue;const f=m.faces[faceIndex];if(!Array.isArray(f)||f.length<3)continue;const positions=[];for(let i=1;i<f.length-1;i++)for(const vi of[f[0],f[i],f[i+1]]){const v=m.vertices[vi];if(v)positions.push(v.x,v.y,v.z);}if(!positions.length)continue;const g=new THREE.BufferGeometry();g.setAttribute('position',new THREE.Float32BufferAttribute(positions,3));const mat=new THREE.MeshBasicMaterial({side:THREE.DoubleSide}),picker=new THREE.Mesh(g,mat);picker.userData.faceIndex=faceIndex;pickers.push(picker);}const hit=raycaster.intersectObjects(pickers,false)[0],faceIndex=Number.isInteger(hit?.object?.userData?.faceIndex)?hit.object.userData.faceIndex:null;pickers.forEach(p=>{p.geometry.dispose();p.material.dispose();});return faceIndex;}
