...
function findThroughPlan(m,sourceFaceIndex,distance){
  ...
  for(let targetFaceIndex=0;targetFaceIndex<m.faces.length;targetFaceIndex++){
    ...
    let useTargetLoop=false,sharedEdge=null,crossBoundary=null,mismatchInterior=false;
    if(allInside){
    ...
    const candidate={sourceFaceIndex,targetFaceIndex,distance:t,projected,useTargetLoop,sharedEdge,crossBoundary,mismatchInterior};if(!best||Math.abs(t)<Math.abs(best.distance))best=candidate;
  }
  return best;
}
...
function buildThroughOnClone(before,plan){
  ...
  const trial=before.clone(),sourceLoop=[...(trial.faces[plan.sourceFaceIndex]||[])],targetLoop=[...(trial.faces[plan.targetFaceIndex]||[])];if(sourceLoop.length<3||typeof trial.bridgeLoops!=='function')return null;
