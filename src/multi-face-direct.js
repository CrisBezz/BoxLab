...
function classifySingleFaceContact(m,sourceFaceIndex,distance){const throughPlan=findThroughPlan(m,sourceFaceIndex,distance),shellHit=firstShellHit(m,sourceFaceIndex,distance);if(throughPlan&&(!shellHit||shellHit.faceIndex===throughPlan.targetFaceIndex))return{mode:'through',throughPlan,shellHit};if(shellHit)return{mode:'blocked',throughPlan:null,shellHit};return{mode:'extrude',throughPlan:null,shellHit:null};}
...
function buildThroughOnClone(before,plan){
  const trial=before.clone(),sourceLoop=[...(trial.faces[plan.sourceFaceIndex]||[])],targetLoop=[...(trial.faces[plan.targetFaceIndex]||[])];
  if(sourceLoop.length<3||sourceLoop.length!==targetLoop.length||typeof trial.bridgeLoops!=='function')return null;
  let innerLoop,ring=null,remainder=null;
  if(plan.useTargetLoop)innerLoop=[...targetLoop];
  else if(plan.sharedEdge){
    const s=plan.sharedEdge;
    innerLoop=plan.projected.map((point,i)=>{
      if(i===s.innerA)return targetLoop[s.targetA];
      if(i===s.innerB)return targetLoop[s.targetB];
      trial.vertices.push(point.clone());return trial.vertices.length-1;
    });
    const ta=s.targetA,tb=s.targetB,ia=s.innerA,ib=s.innerB;
    const outerPath=s.targetForward?pathForward(targetLoop,tb,ta):pathForward(targetLoop,ta,tb);
    const innerStart=s.targetForward?ia:ib,innerEnd=s.targetForward?ib:ia;
    const directForward=(innerStart+1)%innerLoop.length===innerEnd;
    const innerPath=directForward?pathForward(innerLoop,innerEnd,innerStart):pathForward(innerLoop,innerStart,innerEnd);
    if(!outerPath||!innerPath)return null;
    const face=[...outerPath,...innerPath.slice(1,-1)];
    if(face.length<3||new Set(face).size!==face.length)return null;
    remainder=face;
  }else innerLoop=plan.projected.map(point=>{trial.vertices.push(point.clone());return trial.vertices.length-1;});
  for(const index of[plan.sourceFaceIndex,plan.targetFaceIndex].sort((a,b)=>b-a))trial.faces.splice(index,1);
  if(remainder)trial.faces.push(remainder);
  else if(!plan.useTargetLoop){ring=trial.bridgeLoops(targetLoop,innerLoop);if(!ring)return null;}
  const tunnel=trial.bridgeLoops(sourceLoop,innerLoop);if(!tunnel)return null;
  trial.edges();return{mesh:trial,ring,tunnel,remainder};
}
