...
function findThroughPlan(m,sourceFaceIndex,distance){
  ...
  for(let targetFaceIndex=0;targetFaceIndex<m.faces.length;targetFaceIndex++){
    ...
    let useTargetLoop=false,sharedEdge=null,crossBoundary=null;
    if(allInside&&target.length===source.length){
      ...
      crossBoundary=crossBoundaryPlan(m,source,sourceNormal,target,targetFaceIndex,t,projected,outer2,inner2,tol);
      if(!crossBoundary)continue;
    }else continue;
    const candidate={sourceFaceIndex,targetFaceIndex,distance:t,projected,useTargetLoop,sharedEdge,crossBoundary};if(!best||Math.abs(t)<Math.abs(best.distance))best=candidate;
  }
  return best;
}
...
function classifySingleFaceContact(m,sourceFaceIndex,distance){const throughPlan=findThroughPlan(m,sourceFaceIndex,distance),shellHit=firstShellHit(m,sourceFaceIndex,distance);if(throughPlan){const allowed=new Set([throughPlan.targetFaceIndex,throughPlan.crossBoundary?.sideFaceIndex].filter(Number.isInteger));if(!shellHit||allowed.has(shellHit.faceIndex))return{mode:'through',throughPlan,shellHit};}if(shellHit)return{mode:'blocked',throughPlan:null,shellHit};return{mode:'extrude',throughPlan:null,shellHit:null};}
...
function buildThroughOnClone(before,plan){
  ...
  const trial=before.clone(),sourceLoop=[...(trial.faces[plan.sourceFaceIndex]||[])],targetLoop=[...(trial.faces[plan.targetFaceIndex]||[])];if(sourceLoop.length<3||sourceLoop.length!==targetLoop.length||typeof trial.bridgeLoops!=='function')return null;
