...
document.addEventListener('pointerdown',event=>{if(!armed||event.target!==canvas||!event.isPrimary)return;const m=mesh(),camera=state()?.camera;if(!m||!camera)return;const selectionBefore=faces(),allFaceIds=m.faces.map((_,index)=>index),hit=hitSelectedFace(event,m,allFaceIds,camera);if(!Number.isInteger(hit))return;const hitWasSelected=selectionBefore.includes(hit),workingFaces=hitWasSelected?[...selectionBefore]:[...selectionBefore,hit],group=armed==='extrude'?selectionComponentsInfo(m,workingFaces):m?.faceRegionsInfo?.(workingFaces);if(!group)return;const region=group.regions.find(r=>r.faceIndices.includes(hit))||group.regions[0],hitFace=m.faces[hit],worldNormal=(armed==='extrude'?m.faceNormal(hit):region?.normal||m.faceRegionNormal?.(region?.faceIndices||[]))?.clone?.().normalize?.(),controlRegion=armed==='extrude'?{normal:worldNormal,regionVertices:[...hitFace]}:region,regionCenter=centerOf(m,controlRegion.regionVertices);event.preventDefault();event.stopImmediatePropagation();drag={id:event.pointerId,x:event.clientX,y:event.clientY,tool:armed,m,before:m.clone(),faces:[...workingFaces],selectionBefore:[...selectionBefore],hitFaceIndex:hit,hitWasSelected,normal:projectedNormal(m,controlRegion,camera),worldNormal,regionCenter,camera,changed:false,preview:false,snap:null,throughPlan:null,blocked:false,shellHit:null,preparedThrough:armed==='extrude'&&workingFaces.length===1?planThrough(m,workingFaces[0]):null};canvas.setPointerCapture?.(event.pointerId);},true);
