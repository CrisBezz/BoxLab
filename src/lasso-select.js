...
function segmentsIntersect(a,b,c,d){const e=1e-6,o1=orientation(a,b,c),o2=orientation(a,b,d),o3=orientation(c,d,a),o4=orientation(c,d,b);return((o1>e&&o2<-e)||(o1<-e&&o2>e))&&((o3>e&&o4<-e)||(o3<-e&&o4>e));}
function segmentHitsPoly(a,b,poly){if(pointInPoly(a,poly)||pointInPoly(b,poly))return true;for(let i=0;i<poly.length;i++)if(segmentsIntersect(a,b,poly[i],poly[(i+1)%poly.length]))return true;return false;}
function polygonHitsLasso(points,poly){if(points.some(p=>pointInPoly(p,poly)))return true;for(let i=0;i<points.length;i++)if(segmentHitsPoly(points[i],points[(i+1)%points.length],poly))return true;const c=points.reduce((s,p)=>({x:s.x+p.x/points.length,y:s.y+p.y/points.length}),{x:0,y:0});return pointInPoly(c,poly);}
...
function objectsFor(type){const s=state();if(type==='vertex')return[...(s?.vertexObjects?.values?.()||[])];if(type==='edge')return[...(s?.edgeObjects?.values?.()||[])];if(type==='face')return[...(s?.faceObjects?.values?.()||[])];return[];}
function visibleComponent(type,index,p){const camera=state()?.camera,objects=objectsFor(type).filter(Boolean);if(!camera||!objects.length)return true;setRayAt(p);const hit=raycaster.intersectObjects(objects,false)[0];return hit?.object?.userData?.index===index;}
function representative(type,index,m){if(type==='vertex')return screenPoint(m.vertices[index]);if(type==='edge'){const e=m.edges()[index];if(!e)return null;return screenPoint(m.vertices[e.a].clone().lerp(m.vertices[e.b],.5));}if(type==='face'){const f=m.faces[index];if(!f?.length)return null;const c=new THREE.Vector3();f.forEach(id=>c.add(m.vertices[id]));return screenPoint(c.multiplyScalar(1/f.length));}return null;}
function componentHits(type,index,m,poly){if(type==='vertex'){const p=screenPoint(m.vertices[index]);return p&&pointInPoly(p,poly);}if(type==='edge'){const e=m.edges()[index];if(!e)return false;const a=screenPoint(m.vertices[e.a]),b=screenPoint(m.vertices[e.b]);return a&&b&&segmentHitsPoly(a,b,poly);}if(type==='face'){const f=m.faces[index];if(!f?.length)return false;const pts=f.map(id=>screenPoint(m.vertices[id])).filter(Boolean);return pts.length===f.length&&polygonHitsLasso(pts,poly);}return false;}
function componentSelection(poly,type){const m=state()?.mesh;if(!m)return[];const count=type==='vertex'?m.vertices.length:type==='edge'?m.edges().length:type==='face'?m.faces.length:0,out=[];for(let i=0;i<count;i++){if(!componentHits(type,i,m,poly))continue;if(depth()==='visible'){const p=representative(type,i,m);if(!p||!visibleComponent(type,i,p))continue;}out.push(i);}return out;}
function objectCenter(object){const m=object.id===manager()?.activeId?state()?.mesh:object.mesh;if(!m?.vertices?.length)return null;const box=new THREE.Box3().setFromPoints(m.vertices);return box.getCenter(new THREE.Vector3());}
...
function setArmed(next){armed=!!next;button?.classList.toggle('active',armed);clearDraw();gesture=null;if(status)status.textContent=armed?`Lasso • draw around ${mode()} • ${depth()}`:`${mode().charAt(0).toUpperCase()+mode().slice(1)} mode`;}

const button=document.createElement('button');button.id='lassoSelectBtn';button.type='button';button.textContent='Lasso';button.title='Draw a lasso around components or objects';dock?.appendChild(button);button.addEventListener('click',()=>setArmed(!armed));
...
for(const b of document.querySelectorAll('#selectionModes button,#paintSelectDepth [data-paint-depth]'))b.addEventListener('click',()=>{if(armed)queueMicrotask(()=>{if(status)status.textContent=`Lasso • draw around ${mode()} • ${depth()}`;});});

canvas?.addEventListener('pointerdown',event=>{if(!armed||!event.isPrimary||event.button>0)return;event.preventDefault();event.stopImmediatePropagation();gesture={id:event.pointerId,points:[{x:event.clientX,y:event.clientY}],start:{x:event.clientX,y:event.clientY}};canvas.setPointerCapture?.(event.pointerId);draw(gesture.points);},true);
