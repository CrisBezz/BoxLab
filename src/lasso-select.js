...
function componentHits(type,index,m,poly){if(type==='vertex'){const p=screenPoint(m.vertices[index]);return !!(p&&pointInPoly(p,poly));}if(type==='edge'){const e=m.edges()[index];if(!e)return false;const a=screenPoint(m.vertices[e.a]),b=screenPoint(m.vertices[e.b]);return !!(a&&b&&segmentHitsPoly(a,b,poly));}if(type==='face'){const p=representative(type,index,m);return !!(p&&pointInPoly(p,poly));}return false;}
function componentSelection(poly,type){const m=state()?.mesh;if(!m)return[];const count=type==='vertex'?m.vertices.length:type==='edge'?m.edges().length:type==='face'?m.faces.length:0,out=[];for(let i=0;i<count;i++){if(!componentHits(type,i,m,poly))continue;if(depth()==='visible'){const world=representativeWorld(type,i,m),p=world&&screenPoint(world);if(!world||!p||!visibleWorldPoint(world,p))continue;}out.push(i);}return out;}
