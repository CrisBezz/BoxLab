// BoxLab v0.36.18.199 — Studio key + fill + rim light controls.
...
const RIM_INTENSITY_KEY='boxlab-studio-rim-light-intensity';
...
let rimIntensity=Number(localStorage.getItem(RIM_INTENSITY_KEY));
if(!Number.isFinite(rimIntensity))rimIntensity=100;
rimIntensity=Math.max(25,Math.min(200,rimIntensity));
...
function syncUI(){
  ...
  const rimSlider=document.querySelector('#studioRimLightIntensity');
  const rimOutput=document.querySelector('#studioRimLightIntensityOut');
  ...
  if(rimSlider&&Number(rimSlider.value)!==rimIntensity)rimSlider.value=String(rimIntensity);
  if(rimOutput)rimOutput.textContent=`${Math.round(rimIntensity)}%`;
}
...
function setRimIntensity(next,{silent=false}={}){
  const parsed=Number(next);rimIntensity=Math.max(25,Math.min(200,Number.isFinite(parsed)?parsed:100));localStorage.setItem(RIM_INTENSITY_KEY,String(rimIntensity));syncUI();refresh();
  if(!silent){document.dispatchEvent(new CustomEvent('boxlab-studio-rim-light-intensity-change',{detail:{rimIntensity}}));const status=document.querySelector('#selectionStatus');if(status)status.textContent=`Studio Rim Light • ${Math.round(rimIntensity)}%`;}
}
function ensureUI(){
  const section=document.querySelector('#viewportDisplaySection');if(!section)return false;
  if(document.querySelector('#studioLightAngle')&&document.querySelector('#studioLightElevation')&&document.querySelector('#studioLightIntensity')&&document.querySelector('#studioFillLightIntensity')&&document.querySelector('#studioRimLightIntensity')){syncUI();return true;}
  ...
  let rimRow=document.querySelector('#studioRimLightIntensityRow');if(!rimRow){rimRow=document.createElement('label');rimRow.className='range-row';rimRow.id='studioRimLightIntensityRow';rimRow.innerHTML='<span>Rim</span><input id="studioRimLightIntensity" type="range" min="25" max="200" value="100" step="5"/><output id="studioRimLightIntensityOut">100%</output>';section.append(rimRow);const slider=rimRow.querySelector('#studioRimLightIntensity');slider.addEventListener('input',()=>setRimIntensity(slider.value));slider.addEventListener('change',()=>setRimIntensity(slider.value));}
