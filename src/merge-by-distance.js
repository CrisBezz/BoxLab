...
function areaVector(m,face,positions){
  ...
  for(let i=0;i<face.length;i++){
    const a=positions.get(face[i])||m.vertices[face[i]],b=positions.get(face[(i+1)%face.length])||m.vertices[face[(i+1)%face.length];
    if(!a||!b)return null;
...
function plan(m,ids,tolerance){
  ...
  for(const face of nextFaces){const sig=faceSignature(face);if(sigs.has(sig))return{ok:false,reason:'Merge would create duplicate faces'};sigs.add(sig);
  ...
  return{ok:true,tolerance:tol,clusters,representative,positions,nextFaces,merged:selected.length-[...groups.values()].filter(g=>g.length===1).length-clusters.length};
}
...
function apply(){
  ...
  const used=new Set(m.faces.flat()),map=new Map(),vertices=[];m.vertices.forEach((v,i)=>{if(used.has(i)){map.set(i,vertices.length);vertices.push(v.clone());}});
