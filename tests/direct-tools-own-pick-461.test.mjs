...
test('462 supersedes 461 self-pick experiment with proven selected-face drag path',()=>{
  assert.match(face,/const ids=faces\(\);if\(!ids\.length\)return/);
  assert.match(face,/hitSelectedFace\(event,m,ids,camera\)/);
});

test('462 allows first Pencil contact to select a Face while Face tools are armed',()=>{
  assert.doesNotMatch(paint,/#extrudeBtn\.boxlab-direct-stable/);
  assert.doesNotMatch(paint,/#insetBtn\.boxlab-direct-stable/);
});

test('462 keeps Bevel as a self-owning direct edge tool',()=>{
  assert.match(paint,/#bevelBtn\.active/);
});

test('462 removes the 460 coordination shim from live runtime',()=>{
  assert.doesNotMatch(index,/direct-tool-ownership-460\.js/);
});

test('462 legacy main drag still yields to mature direct tools',()=>{
  assert.match(main,/#extrudeBtn\.boxlab-direct-stable,#insetBtn\.boxlab-direct-stable,#bevelBtn\.active/);
});

test('462 restores Face controller pin and preserves other critical pins',()=>{
