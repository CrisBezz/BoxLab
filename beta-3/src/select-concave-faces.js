// BoxLab v0.36.18.66 — non-destructive concave Face inspection.
// Selects polygon faces with at least one reflex/inward corner after projection
// to the face's dominant plane. Geometry/history are untouched.
...
button.id='selectConcaveFacesBtn';
button.type='button';
button.textContent='Concave';
...
function inspect(m){
  ...
  const indices=[];
  (m.faces||[]).forEach((face,index)=>{if(isConcave(m,face))indices.push(index);});
...
function apply(){
  ...
  queueMicrotask(()=>{
    ...
    if(status)status.textContent=info.count
      ?`Concave Faces • ${info.count} face${info.count===1?'':'s'} selected`
      :'Topology check • 0 concave faces';
  });
}
...
function sync(){
  ...
  button.title=info.count
    ?`Select ${info.count} concave face${info.count===1?'':'s'}`
    :'No concave faces found';
}
...
globalThis.__boxlabSelectConcaveFaces={version:'0.36.18.66',inspect,apply,isConcave};
