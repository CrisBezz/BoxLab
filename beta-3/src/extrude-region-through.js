...
function legacyCornerPattern(m,sourceFaceIndex,targetFaceIndex,projected,tol){const source=m.faces[sourceFaceIndex],target=m.faces[targetFaceIndex];if(source?.length<4||target?.length<4)return false;const hits=projected.map(p=>target.findIndex(id=>p.distanceTo(m.vertices[id])<=tol*2.5)),matched=hits.filter(i=>i>=0);if(matched.length!==1)return false;const cornerSlot=hits.findIndex(i=>i>=0),prev=(cornerSlot-1+source.length)%source.length,next=(cornerSlot+1)%source.length,normal=m.faceNormal(targetFaceIndex).clone().normalize(),{u,v}=faceBasis(normal),origin=m.vertices[target[0]],to2=p=>new THREE.Vector2(p.clone().sub(origin).dot(u),p.clone().sub(origin).dot(v)),outer=target.map(id=>to2(m.vertices[id])),boundary=p=>outer.some((a,i)=>pointSegDistance2(p,a,outer[(i+1)%outer.length])<=tol*2.5);return boundary(to2(projected[prev]))&&boundary(to2(projected[next]));}
...
function regionPlan(m,sourceFaceIndex){
  ...
  for(let fi=0;fi<m.faces.length;fi++){
    ...
    if(targetFaces.length===1&&legacyCornerPattern(m,sourceFaceIndex,fi,projected,tol))continue;
    ...
    for(let i=0;i<source.length;i++){
      ...
      if(hitFaces.length)exteriorEdges.push({slot:i,faces:hitFaces,normal:sideNormal,point:sidePoint});
    }
    const candidate={sourceFaceIndex,distance:t,projected,targetSeed:fi,targetFaces,targetNormal,planePoint,tol,exteriorEdges};
...
window.addEventListener('pointerdown',event=>{if(internalCancel||event.target!==canvas||!event.isPrimary||!extrudeArmed())return;const m=mesh(),faceIndex=selectedFace(),camera=state()?.camera;if(!m||!Number.isInteger(faceIndex)||!camera)return;const plan=regionPlan(m,faceIndex);if(!plan)return;const normal2d=projectedNormal(m,faceIndex,camera);if(!normal2d)return;probe={id:event.pointerId,x:event.clientX,y:event.clientY,m,faceIndex,before:m.clone(),plan,normal2d,passedMove:false};},true);
window.addEventListener('pointermove',event=>{if(internalCancel||!probe||probe.id!==event.pointerId||takeover)return;const dx=event.clientX-probe.x,dy=event.clientY-probe.y;if(Math.hypot(dx,dy)<8)return;const distance=(dx*probe.normal2d.x+dy*probe.normal2d.y)*.006,toward=Math.sign(distance)===Math.sign(probe.plan.distance);if(toward&&Math.abs(distance)>=Math.abs(probe.plan.distance)*.55){event.preventDefault();event.stopImmediatePropagation();if(!probe.passedMove)globalThis.__boxlabHistory?.push(probe.before);cancelNativeDrag(probe.id);takeover=probe;probe=null;let d=distance,ready=false;if(Math.abs(d)>=Math.abs(takeover.plan.distance)){d=takeover.plan.distance;ready=true;}takeover.distance=d;takeover.ready=ready;previewExtrude(takeover.m,takeover.before,takeover.faceIndex,d);if(status)status.textContent=`${ready?'THROUGH REGION READY':'Extrude In'} • ${takeover.plan.targetFaces.length} target face${takeover.plan.targetFaces.length===1?'':'s'} • ${d>=0?'+':''}${d.toFixed(2)}`;render();}else probe.passedMove=true;},true);
window.addEventListener('pointermove',event=>{if(internalCancel||!takeover||takeover.id!==event.pointerId)return;event.preventDefault();event.stopImmediatePropagation();const dx=event.clientX-takeover.x,dy=event.clientY-takeover.y;let d=(dx*takeover.normal2d.x+dy*takeover.normal2d.y)*.006,ready=Math.sign(d)===Math.sign(takeover.plan.distance)&&Math.abs(d)>=Math.abs(takeover.plan.distance);if(ready)d=takeover.plan.distance;takeover.distance=d;takeover.ready=ready;previewExtrude(takeover.m,takeover.before,takeover.faceIndex,d);if(status)status.textContent=`${ready?'THROUGH REGION READY':d<0?'Extrude In':'Extrude'} • ${takeover.plan.targetFaces.length} target face${takeover.plan.targetFaces.length===1?'':'s'} • ${d>=0?'+':''}${d.toFixed(2)}`;render();},true);
window.addEventListener('pointerup',event=>{if(internalCancel)return;if(takeover&&takeover.id===event.pointerId){event.preventDefault();event.stopImmediatePropagation();const t=takeover;takeover=null;probe=null;if(t.ready){const built=buildRegion(t.before,t.plan);if(built){restore(t.m,built);bridge()?.set?.('face',[]);if(status)status.textContent=`Extrude Through • shell region rebuilt • ${t.plan.targetFaces.length} target face${t.plan.targetFaces.length===1?'':'s'}`;}else{restore(t.m,t.before);if(status)status.textContent='Extrude Through • shell region rebuild failed';}}else{previewExtrude(t.m,t.before,t.faceIndex,t.distance);bridge()?.set?.('face',[t.faceIndex]);}render();return;}probe=null;},true);
