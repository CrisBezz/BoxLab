// BoxLab v0.36.18.289 — surface-flow continuity scoring for guarded open-chain all-quad Bridge.
...
function bestEqualQuadPlan(mesh,chainA,chainB){
  ...
  for(const reverse of[false,true]){
    ...
    for(const flip of[false,true]){
      ...
      if(!best||score<best.score)best={faces,mapped,reverse,flip,windingPenalty:winding,surfacePenalty:surface,flowPenalty:flow.penalty,connectorFlowPenalty:flow.connectorPenalty,quadAspectPenalty:flow.aspectPenalty,surfaceFlowPenalty:flow.surfacePenalty,score,qualityGuarded:true};
    }
  }
...
export function installOpenChainAllQuadBridge(EditableMesh){
  const proto=EditableMesh?.prototype;if(!proto||proto.__openChainAllQuadBridge289Installed)return;
  ...
  proto.bridgeSelectedEdges=function(edgeIndices){
    ...
    const result={faceIndices,plan:{...best.plan,quadCount:faceIndices.length,triangleCount:0,qualityGuarded:true,correspondenceSearch:true,searchCandidates:tested,searchAttempts:attempted,searchPhase:best.phase,reverseShort:best.reverseShort,flowPenalty:best.plan.flowPenalty??0,connectorFlowPenalty:best.plan.connectorFlowPenalty??0,quadAspectPenalty:best.plan.quadAspectPenalty??0,surfaceFlowPenalty:best.plan.surfaceFlowPenalty??0},openChain:true,unequal:true,allQuad:true,subdFriendly:true,balancedDensification:true,qualityGuarded:true,correspondenceSearch:true,searchCandidates:tested,searchAttempts:attempted,searchPhase:best.phase,addedVertices,denseCounts:[best.denseA.length-1,best.denseB.length-1]};
    diagnostic(true,null,{addedVertices,denseCounts:result.denseCounts,connectors:best.quality.connectors,searchCandidates:tested,searchAttempts:attempted,searchPhase:best.phase,reverseShort:best.reverseShort,searchScore:best.score,flowPenalty:best.plan.flowPenalty??0,connectorFlowPenalty:best.plan.connectorFlowPenalty??0,quadAspectPenalty:best.plan.quadAspectPenalty??0,surfaceFlowPenalty:best.plan.surfaceFlowPenalty??0});
    return result;
  };

  proto.__openChainAllQuadBridge289Installed=true;
