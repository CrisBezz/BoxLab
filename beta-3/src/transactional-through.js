...
function coplanarFaces(m,seed,normal,point,tol){return m.faces.map((_,fi)=>samePlane(m,fi,normal,point,tol)?fi:-1).filter(fi=>fi>=0);}
...
function throughPlan(m,sourceFaceIndex){
  ...
  for(let fi=0;fi<m.faces.length;fi++){
    ...
    const projected=source.map(id=>m.vertices[id].clone().addScaledVector(sourceNormal,distance));
    const targetFaces=coplanarFaces(m,fi,plane.normal,plane.point,tol*3);
    ...
    // Existing holes are allowed: only reject when all sampled points miss the coplanar shell region.
    const hits=samples.filter(p=>pointInRegion(m,targetFaces,p,plane.normal,plane.point,tol*2)).length;
    if(hits<Math.max(1,Math.ceil(samples.length*.35)))continue;
...
function splitFaceByProjectedPolygon(m,fi,poly3,cache,tol){
  const face=m.faces[fi];if(!Array.isArray(face)||face.length<3)return null;const normal=m.faceNormal(fi)?.clone().normalize();if(!normal)return null;
  const {u,v}=faceBasis(normal),origin=m.vertices[face[0]].clone(),to2=p=>new THREE.Vector2(p.clone().sub(origin).dot(u),p.clone().sub(origin).dot(v)),to3=p=>origin.clone().addScaledVector(u,p.x).addScaledVector(v,p.y);
  let outer=face.map(id=>to2(m.vertices[id])),hole=poly3.map(to2);if(Math.abs(polygonArea(hole))<1e-10)return null;
  if(polygonArea(outer)<0)outer.reverse();if(polygonArea(hole)>0)hole.reverse();
  const tris=THREE.ShapeUtils.triangulateShape(outer,[hole]);if(!tris?.length)return null;
  const combined=[...outer,...hole],faces=[];for(const tri of tris){const ids=tri.map(i=>getVertex(m,cache,to3(combined[i]),tol));if(new Set(ids).size===3)faces.push(orientLike(m,ids,normal));}
  return faces;
}
...
function buildThroughOnClone(before,plan){
  ...
  const opening=plan.projected.map(p=>getVertex(trial,cache,p,plan.tol));
  const replacementFaces=[];
  // Rebuild every coplanar target face that intersects the opening. Faces which do not intersect remain untouched.
  const affected=[];
  for(const fi of plan.targetFaces){
    const face=trial.faces[fi];if(!Array.isArray(face)||face.length<3)continue;
    const normal=trial.faceNormal(fi)?.clone().normalize();if(!normal)continue;const {u,v}=faceBasis(normal),origin=trial.vertices[face[0]],to2=p=>new THREE.Vector2(p.clone().sub(origin).dot(u),p.clone().sub(origin).dot(v));
    const face2=face.map(id=>to2(trial.vertices[id])),hole2=plan.projected.map(to2),sampleHit=hole2.some(p=>pointInPolygonInclusive(p,face2,plan.tol*2))||face2.some(p=>pointInPolygonInclusive(p,hole2,plan.tol*2));
    if(!sampleHit)continue;
    const pieces=splitFaceByProjectedPolygon(trial,fi,plan.projected,cache,plan.tol);if(!pieces)return{ok:false,reason:'target-rebuild-failed',face:fi};affected.push(fi);replacementFaces.push(...pieces);
  }
